...
CKEDITOR.dialog.add('xd_calc_property',function(editor){
	...
	return {
		...
		onShow : function(){
			delete this.xd_calc_property;

			var element = this.getParentEditor().getSelection().getSelectedElement();

			if ( element )
			{
				this.xd_calc_property = element;
				this.setupContent( element );
				
				this.getContentElement('xd_calc_property','calcName').setValue(element.getAttribute('title'));
				this.getContentElement('xd_calc_property','calcValue').setValue(element.getAttribute('value'));
				this.getContentElement('xd_calc_property','calcPrec').setValue(element.getAttribute('prec'));
				...
				for(var i = 0; i < styleArray.length; i++){
					...
					if (itemArray[0] === 'font-size') {
						this.getContentElement('xd_calc_property','calcFontSize').setValue(itemArray[1].substr(0,(itemArray[1].length - 2)));
					}
					if(itemArray[0] === 'width'){
						this.getContentElement('xd_calc_property','calcWidth').setValue(itemArray[1].substr(0,(itemArray[1].length - 2)));
					}
					if(itemArray[0] === 'height'){
						this.getContentElement('xd_calc_property','calcHeight').setValue(itemArray[1].substr(0,(itemArray[1].length - 2)));
					}
				}
				
				if(typeof(MODULE_CONFIG) !== 'undefined'){
					this.getContentElement('xd_calc_property','module_field').setValue(element.getAttribute('module_field'));
				}
			}
		},
		onOk : function(){
			var editor,
				element = this.xd_calc_property,
			...
			element.setAttribute('title',this.getContentElement('xd_calc_property','calcName').getValue());
			element.setAttribute('value',this.getContentElement('xd_calc_property','calcValue').getValue());
			element.setAttribute('prec',this.getContentElement('xd_calc_property','calcPrec').getValue());
			
			var styleString = '';
			var calcuFontSize = this.getContentElement('xd_calc_property','calcFontSize').getValue();
			var calcuWidth = this.getContentElement('xd_calc_property','calcWidth').getValue();
			var calcuHeight = this.getContentElement('xd_calc_property','calcHeight').getValue();
			if(calcuFontSize){
			...
			element.setAttribute('element_type','xd_calc_property');
			
			if(typeof(MODULE_CONFIG) !== 'undefined'){
				element.setAttribute('module_field',this.getContentElement('xd_calc_property','module_field').getValue());
			}
