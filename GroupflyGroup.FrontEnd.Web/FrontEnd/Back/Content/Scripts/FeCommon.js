...
function SetImgAutoSize(obj) {
    var img = $(obj);
    var MaxWidth = 228; //设置图片宽度界限
    var MaxHeight = 288; //设置图片高度界限
    var MaxHeightWidth = MaxHeight / MaxWidth;
    var HeightWidth = img.height() / img.width(); //img.offsetHeight / img.offsetWidth; //设置高宽比
    //console.log("width:" + img.width() + ",height:" + img.height() + ",maxwidth:" + MaxWidth + ",maxheight:" + MaxHeight + ",heightwidth:" + HeightWidth + ",maxheightwidth:" + MaxHeightWidth + ".");
    //宽图
    if (MaxHeightWidth >= HeightWidth) {
        //img.css("width", MaxWidth).css("height", MaxWidth * HeightWidth).css("margin-top", (MaxHeight - MaxWidth * HeightWidth) / 2);
        img.css({ "width": MaxWidth, "height": MaxWidth * HeightWidth, "margin-top": (MaxHeight - MaxWidth * HeightWidth) / 2 });
    } else {
        //img.css({ "width": MaxHeight / HeightWidth, "height": MaxHeight, "margin-left": (MaxWidth - MaxHeight / HeightWidth) / 2 });
        img.css("width", MaxHeight / HeightWidth).css("height", MaxHeight).css("margin-left", (MaxWidth - MaxHeight / HeightWidth) / 2);
    }
}
